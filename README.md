...
1. Open Play, choose **Hand space** in **Play with**, and press **Start audio**. The unattended sound is instrumental.
...
The mixer now defaults to **Simulation** in Play: the same full-screen physics simulations developed in `sim.html` drive the live music through their published signals. Open **Simulation controls** to choose Trails, Veil, Basin, Prism, or Presence and an input. **Sound mappings & performance patch** connects their signals to engagement, stem balance, and echo/reverb, with editable ranges and smoothing. Start audio after choosing your music; Fullscreen keeps the picture and playback controls together. Hand space and Manual controls remain available in **Play with**.

Develop visuals separately at **http://127.0.0.1:4178/sim.html** after `npm run dev`. Tune a simulation, name its performance patch, and press **Play in mixer** to bring the configuration and mappings across. **Save patch** retains it in this browser; **Export patch** and **Open patch** move it between machines or back to the studio. Both pages run one shared simulation implementation. Music stays in the existing library/manifest workflow.

Try `/?fixtures=1&play=simulation&sim=presence&source=synthetic&quality=low` for an explicit test-tone demonstration, or `/?play=space` for the legacy hand preview. The depth-camera bridge lives in `bridge/`. `telemetry.html` and `npm run telemetry:sink` remain available for external monitoring. See [the architecture and patch workflow](docs/ARCHITECTURE.md) and [simulation development documentation](docs/SIMULATIONS.md).

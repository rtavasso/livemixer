...
- **Rate.** The controller delivers up to ~100 stereo pairs per second; 3-way SGBM on 480x360 takes 13-17 ms on the development desktop (five-direction SGBM on the old 320x240 view took the same 14 ms, on 480x360 42-44 ms), so the bridge takes the newest pair each time it is ready and paces itself to `--fps` (default 30). Cost is proportional to `width * height * disparities`, and the disparity range to `f / near`: `--near 0.15` is a fifth cheaper than the default 0.1. The post-processing that keeps a far hand usable (a 5x5 disparity median, `--leap-fill-radius 2`, `--leap-temporal 3`; *Distance* below) adds about 10 ms on the same desktop while it is loaded with other work, 3-5 ms of it the temporal median.
- **Knowing when the data is bad.** Every frame's `stats` carries `depthValidFraction` (the share of the foreground's hull, in-range pixels closed by 7 px, that holds a measurement: 1 for a solid scan, well below for a hand full of holes) and `depthNoiseMm` (the median local 5x5 standard deviation of the in-range depth: a few mm at 30 cm, 10 and more when the scan has turned to noise), so the overlay and the telemetry can show it. `leap_source.py --dump-images` prints both per frame.
...
If what you actually want is height above the device to be *up* and reaching over the device toward the display to be *into* the scene, none of this is needed: run with `--frame upright` (*Camera on the desk looking up*, below), which re-orients the box in the bridge, metrically, so the browser's default mapping is right.
...
| `--leap-block N` / `--leap-mode M` / `--leap-matcher M` | Block size (5), SGBM path aggregation (`3way`, `sgbm`, `hh4`, `hh`) and matcher (`sgbm`, `bm`). |
| `--leap-p1 N` / `--leap-p2 N` | SGBM smoothness penalties per block pixel, `P1 = N * block²` for a 1 px disparity step and `P2` for larger ones (defaults 8 and 32). |
| `--leap-speckle N` | Drop matched patches smaller than N pixels at 320x240, scaled with the view's area (default 200 = 450 px at 480x360); 0 = off. |
| `--leap-contrast MODE` / `--leap-contrast-window N` | Condition the pair before matching: `gain` (divide by the local mean over an N px box, default 31) or `lcn` (local contrast normalisation); default `none`, see *Distance* below for why. |
| `--leap-fill-radius N` | Fill holes up to about 2N px wide from surrounding depth that agrees within 1.5 disparity px (default 2; 0 = off). Does not bridge the gaps between spread fingers. |
| `--leap-temporal N` | Per-pixel median of the last N depth maps where they agree with the newest one within 1.5 disparity px at that depth (default 3; 0 or 1 = off). A hole in the newest frame is filled only when two older frames agree. |
...
- **The lab's table** (this desktop, `--repeat 5`, frame 001 = the open hand at 27 cm; "gap" = the fraction of the space between the fingers that reads as near depth, lower is better; fingers = runs of near depth across the finger row vs runs of lit skin):
  ...
  Block size 3 or 7 and the median filter change little; `hh` resolves the fingers best but costs 8x; `bm` is fast and nearly empty on skin. 480x360 with the 3-way pass costs what 320x240 five-direction SGBM did and halves the gap fill; 640x480 halves it again for 2.5x the time, over the ~20 ms budget of a 2020 Intel MacBook. The fused skeleton (*Fusing the skeleton into the scan*) fills the finger gaps the matcher still bridges. (The lab's hand mask has since been tightened to exclude what the reference matcher places far beyond the hand, a lit shelf near the far plane in these frames; the numbers below are on that mask and are a few points different from this table.)
...
python bridge/stereo_lab.py --input test-results/leap --frames 001 --simulate-distance 350 450 550 --temporal 3 --configs 480x360 default   # the far-hand table below
```

#### Distance: the hand at 35-55 cm

A hand a foot to a foot and a half above the device (30-45 cm) is where the scan falls apart in use: it is dim (the LEDs' inverse-square falloff leaves it at a third of its 27 cm grey levels at 45 cm), small (60-90 view px wide, 14 raw rows tall), and each disparity pixel is worth `Z²/(f·B)` = 21 mm at 45 cm against 9 mm at 30 cm. The dumps hold only the 27 cm hand and the room never had a far hand in it when frames were taken, so the lab **simulates distance** from frame 001 (`--simulate-distance Z ...`): every raw image is re-imaged through the device's own calibration (inverted with Newton's method, exact to a hundredth of a pixel) with the ray slopes scaled by `s = 27 cm / Z`, which moves every scene point to `Z0 / s`, scales every disparity by exactly `s`, and lets the real sensor's 2.4 x 1.2 px/deg sample the smaller hand; the hand is dimmed by `s²` and sensor noise is topped up to the 2.1 grey levels the real frames carry. The truth for a simulated frame is the 27 cm frame's own depth (full SGBM, holes filled, smoothed) scaled the same way, so besides the valid fraction, the noise (median local 5x5 standard deviation of the depth over the hand core), the holes (patches of missing depth inside the hand core) and the phantoms, the lab reports the median and 90th-percentile depth error and the **wrong matches** (measurements off by more than 15 % of the truth: the scattered near and far spikes a scan of a far hand is full of). A live dump was taken during this work too (`leap_source.py --dump-images 5`), but nobody was holding a hand over the device: an empty room, 2-3 k in-box speckle pixels a frame.

What the sweep over conditioning, smoothness, uniqueness, block size, path aggregation, speckle and filling found (`stereo_lab.py --list` has every configuration; frame 001 at 27 cm and its simulations at 35, 45 and 55 cm):

- **The far hand is matched, it is wrong.** With the previous defaults the hand core is 65-68 % valid at 27-45 cm and 58 % at 55 cm, so the resolution is not what goes first; the wrong matches do: 3 % of the core at 35 cm, 7 % at 45, 14 % at 55, with the 90th-percentile error going 28 -> 58 -> 93 mm and the local noise 3 -> 6 -> 9 mm. The surface scan takes the nearest depth per cell, so those spikes are exactly what it shows.
- **Input conditioning does not help.** Dividing the rectified pair by its local mean (`--leap-contrast gain`) so a dim hand has a bright one's amplitude was the obvious fix and is not one: it *lowers* the valid fraction at distance (65/67/58 -> 57/47/53 % at 35/45/55 cm with a 31 px window) while cutting the phantoms (9.5/9.2/8.9 -> 7.4/5.9/5.6 %) and leaving the wrong matches where they were; amplifying the data term is the same as weakening SGBM's smoothness, and on skin at 2-3 raw px/deg smoothness is what fills the hand. Local contrast normalisation (`lcn`), which also removes the shading, loses a fifth of the 27 cm hand. Both stay available, off by default. The eyes' brightness was checked as a cause too: over the lit pixels the rectified pair differs by 1 %, so no.
- **Smoothness, uniqueness and path aggregation trade one number for another.** Doubling `P1/P2` (16/64) lifts the 45 cm valid fraction to 71 % and halves the wrong matches there (4 %), but drags the small far hand toward the background (a 15 mm depth bias at 55 cm against 9 mm), adds 1.5 points of phantoms and merges a finger at 35 cm; halving them does the reverse. Uniqueness 10 gives +4 points valid for +1.5 points phantoms and slightly more wrong matches; 20 the reverse. Full SGBM (5 directions) and `hh` (8) reduce the wrong matches by 1-2 points at 45-55 ms and 125 ms; the new `hh4` mode costs 70 ms in this OpenCV build for nothing. Block 7 lowers the noise (4.4 against 5.6 mm at 45 cm) and loses a finger at 35 cm. So the defaults keep 8/32, 15, `3way`, block 5.
- **What does help is free of those trades:** a **5x5 disparity median** instead of 3x3 (holes 57 -> 36 at 27 cm, 42 -> 27 at 45 cm, noise down a tenth, nothing else moves), **hole filling** by normalised convolution where the surrounding depth agrees (`--leap-fill-radius 2`: holes down another third; radius 3 costs a finger at 35 cm, so 2 it is; the 27 cm finger count is unchanged), and the **temporal median** over 3 frames (`--leap-temporal 3`): per pixel the median of the samples that agree with the newest frame within 1.5 disparity px at that depth, so a moving hand keeps its newest sample and nothing lags (10 mm/frame of motion shifts the median by 3 mm); a hole in the newest frame is filled only when two older frames agree there, which is what keeps the room's speckle from being carried forward. It takes the wrong matches from 3/6/14 % to 1/4/9-10 % and the 90th-percentile error from 28/56/92 to 27/52/79-84 mm, and fills a tenth more of the 55 cm hand. The majority variant (a lone newest sample outvoted by two agreeing older ones, `TemporalDepthFilter(majority=True)`) shaves another point of wrong matches and is not worth its cost.

  | config | Z | matcher ms | hand valid % | noise mm | holes | err p50/p90 mm | wrong % | phantom % |
  | --- | --- | --- | --- | --- | --- | --- | --- | --- |
  | before (3x3 median, no fill, no temporal) | 27 cm | 21 | 68 | 1.5 | 57 | - | - | 4.5 |
  | | 35 cm | 22 | 65 | 3.1 | 49 | 7 / 28 | 3 | 9.5 |
  | | 45 cm | 21 | 67 | 6.1 | 42 | 18 / 58 | 7 | 9.2 |
  | | 55 cm | 19 | 58 | 8.8 | 32 | 25 / 93 | 14 | 8.9 |
  | **default** (5x5 median, fill 2) | 27 cm | 24 | 68 | 1.4 | 26 | - | - | 4.5 |
  | | 35 cm | 25 | 66 | 2.9 | 25 | 7 / 28 | 3 | 9.8 |
  | | 45 cm | 22 | 68 | 5.6 | 24 | 18 / 56 | 6 | 9.5 |
  | | 55 cm | 24 | 60 | 8.1 | 16 | 26 / 92 | 14 | 9.2 |
  | **default + temporal 3** (static / moving 10 mm per frame) | 35 cm | +5 | 66 / 67 | 3.1 / 3.5 | 32 / 31 | 6 / 26 · 7 / 27 | 1 / 2 | 9.9 / 10.6 |
  | | 45 cm | +4 | 69 / 66 | 5.9 / 5.9 | 18 / 17 | 14 / 52 · 14 / 52 | 4 / 4 | 11.1 / 11.6 |
  | | 55 cm | +5 | 70 / 73 | 9.7 / 9.8 | 21 / 21 | 28 / 79 · 25 / 84 | 9 / 10 | 9.3 / 10.1 |

  (Development desktop under load; the ms column is the matcher with its rules and filling, `+N` the temporal median's own cost. The noise column rises slightly with the temporal median because filled pixels carry other frames' noise; the errors against the truth fall.)
- **What remains physics.** At 45-55 cm the hand's texture is at the sensor's noise floor in 8 bits (local standard deviation 2.2 against 2.0 of noise), a finger is 4 raw px wide and 2 rows tall, and a disparity pixel is worth 21-31 mm: a median error of 14-25 mm, a 90th percentile of 50-80 mm and a tenth of the pixels wrong is what this pair of cameras can do there, and no matcher setting moves that by more than a few points. The remedy for the scan at that height is the skeleton (*Fusing the skeleton into the scan*): the hand reads as the smooth model with the measurement kept where it is consistent. The lab's simulated errors include a bias that grows with distance (5 mm at 45 cm, 13 mm at 55 cm, more with heavier smoothing): the scaling is done in the aligned frame with the fitted pitch and roll, and a real yaw of about 0.1° between the cameras' calibrations (a 0.4 px disparity offset the fit cannot see) turns into a depth error that grows as `1/s - 1`. A live far hand, when someone holds one over the device during `--dump-images`, is the check.
...
{ "type": "frame", "seq": 12, "t": 1234.567,
  ...
  "stats": { "pixels": 3376, "blobs": 1, "trackedHands": 0, "fusedHands": 0, "scanModelFraction": 0, "fps": 29.8, "processingMs": 6.1, "frameWidth": 640, "frameHeight": 480,
             "depthNoiseMm": 5.8, "depthValidFraction": 0.85 } }   // the last two: Leap sources only, see "Knowing when the data is bad"
```
...
**Modes** (`--scan-fuse`, default `blend`):

| Mode | Under the model | Outside the model |
| --- | --- | --- |
| `blend` | `fill`, made noise-aware: the measurement is first median-filtered (5x5, holes taken as the model), a smoothed value that agrees with the model within `--fuse-tolerance` is mixed with it as `(1 - w) * measured + w * model`, and where the measurement is missing or disagrees the model replaces it. The weight `w` is the larger of two ramps: 0 at 5 mm of local measurement noise and 1 at 20 mm, and 0 at 70 % of the box's depth range and 1 at its far plane (345 -> 450 mm above a controller with the default box: the far third of the box, where its stereo is worst, leans on the model). | The measurement, untouched. |
...
Model pixels outside `--near`/`--far` are dropped like any other pixel, so a hand held outside the box adds nothing, and only hands that are reported (palm within the margin of the box) are rendered. `fill` keeps the real relief of the hand where the stereo did see it and adds the model only where it did not, so a measured palm stays a measured palm and the phantom-free fingers come from the skeleton; `model` is for a device whose depth is too poor to trust at all under the hand, and gives a hand of perfectly smooth tubes. `blend` is the default because of the far hand (*Distance* above): a hand 45 cm above the controller is measured with 5-10 mm of local noise and a tenth of its pixels wrong, and `fill` passes every measurement within the tolerance straight through, spikes included. On a dome measured like that (12 mm of noise, a quarter holes, 7 % wrong by 90 mm) `blend` reads 6.2 mm from the truth on average against 8.7 for `fill` and 6.5 for the tube model (90th percentile 12 / 18 / 13 mm) and is three times smoother (local standard deviation 2.8 against 10.6 mm), and on a clean measurement at 27 cm it is the measurement (2 % of it counts as model). The gain in accuracy depends on how well the model fits the surface: on the same dome in a 400-1200 mm box, where the tube model is 12 mm off it, `blend` is 10 % *less* accurate than `fill` (11.2 against 10.2 mm) and still three times smoother; a tracked hand's capsules fit a real hand to about the 10-15 mm the palm point sits inside it. `--scan-fuse fill` is one flag away. In upright mode the same rule runs on the front view (`scan_fusion.fuse_front`), with the height of each row as the depth for the weight.
...
**Stats.** Every frame reports `stats.fusedHands`, how many hands were rendered into the depth (0 when the frame had no tracked hands or `--scan-fuse off`), and `stats.scanModelFraction`, the fraction of the frame's foreground pixels (after the morphological opening, so exactly the pixels behind `stats.pixels`) whose depth came from the model. It is the single number that says how much of the scan you are looking at is skeleton rather than measurement. Under `fill` it is the size of the holes the matcher left: a few percent for a well-lit hand at 25 cm, most of the hand when it saturates the image close to the device, 0 when tracking is lost. Under `blend` a pixel also counts as model when its weight is at least a half, so it grows with the noise and the distance: a few percent at 27 cm, most of the hand at 45 cm and beyond. Under `model` it is the model's share of the foreground, typically 0.6-0.8 (the hand, not the arm).
...
| Flag | Meaning |
| --- | --- |
| `--scan-fuse MODE` | `blend` (default), `fill`, `model` or `off`, as above. |
| `--fuse-tolerance MM` | Under `fill` and `blend`, how far a measurement may differ from the model and still be kept (default 40). |

## Camera on the desk looking up (`--frame upright`)

Everything above describes the box in the camera's own terms: `u`/`v` follow the image and `w` is the camera's depth, which is right for a camera on a tripod facing the performer. For the controller lying on the desk between the performer and the display it is not: its depth is the *height* above the device, so lifting the hand reads as pushing into the scene, and moving the hand toward the display moves it up or down the screen. Worse, the camera looks up through a frustum, so image-normalized `u`/`v` drift toward the centre as the hand is lifted (a hand 10 cm up spans 20 cm of image, at 45 cm it spans 90 cm). `--frame upright` fixes both, and the browser then needs nothing but its default depth mapping:

```sh
python bridge/depth_bridge.py --source leap --frame upright
```

**Axes.** Each depth pixel `(u px, v px, d mm)` is turned into a metric point with the source's pinhole intrinsics, `X = (u − cx)·d/fx` across the image, `V = (v − cy)·d/fy` down the image and `Y = d` the height above the device, and the box becomes a metric volume centred on the optical axis. The wire layout stays `[u, v, w]` with `u` rightwards, `v` downwards and `w` deeper, but now:

| Output | Meaning | Formula |
| --- | --- | --- |
| `u` | along the device's long axis | `(X − X0 + W/2) / W` |
| `v` | DOWN from the top of the height range: `v = 0` at `--far`, `v = 1` at `--near` | `1 − (Y − near) / (far − near)` |
| `w` | across the device toward the display, `w = 0` at the glass | `(s·V − Z0 + D/2) / D` |

So the browser's default depth mapping (mirror x, flip y so that image-down becomes sim-up, keep `w` as z) already does the right thing: height above the device becomes sim y, reaching over the device toward the display becomes sim z, the device's long axis is sim x; the mirror toggles fix left/right. It is orthographic, so a hand lifted straight up keeps its `u` and `w`. Skeleton joints go through the same maths and their widths become millimetres over `W`, so a finger is the same width at any height. Blobs are still connected components of the depth image, but each is measured from its metric points: `pos` is the centroid of the solid (the underside the camera saw, raised by half the slab), `w` its nearest reach, `extent` spans the slab.

**The metric box.** `--near`/`--far` are the height range in metres (the Leap default `0.1`/`0.45`, 350 mm). `--box-mm W D` (default `620 350`) is the box across the device and toward the display in millimetres, `--box-center X Z` (default `0 0`, millimetres from the optical axis) where its centre sits. Size it to the simulation volume, which is `aspect : 1 : volumeDepth` in units of its height, with the height range as the unit: `W = aspect × (far − near)`, `D = volumeDepth × (far − near)`; the defaults are 16:9 × 350 mm and 1 × 350 mm. Anything outside the box is dropped exactly like today's near/far; `--roi` is only an image crop here (the normalization is metric, not relative to it), and `--box-x`/`--box-y` are ignored.

**Which way is the display.** `--reach-sign +1|-1` (default `+1`): whether image-down (`+1`) or image-up (`-1`) of the depth image faces the display, which depends on how the device lies on the desk. The browser's mirror toggles fix left/right and forward/back too, so the only thing to check is that reaching toward the screen goes into the scene. `--leap-orient` is honoured: the intrinsics are turned with the image.

**The scan is a front view.** The camera sees the hand's underside; the simulations want its front, the face toward the glass. So the surface scan becomes a height field over the FRONT of the box: per `(u, v)` cell the smallest `w` of the solid hand, where the solid is the union of (a) the measured underside extruded UP by `--slab-mm` (default 30: hands and fingers are that thick, and the camera only sees their underside) and (b), when fusion is on and the frame has skeletons, the tracked hands' capsule model rendered orthographically from the front (the same tubes as before, seen from the side: `w − relief`, rasterised on cells of at most 4 mm and reduced into the grid so fingers survive a coarse scan). Fusion happens in that front view under the same modes and `--fuse-tolerance`: the model wins where the measurement is missing or disagrees by more than the tolerance, and `stats.scanModelFraction` is the model's share of the scanned cells. The voxels (`x` = `u`, `y` = `v` top row first, `z` = `w` from the glass, each a fill fraction of the solid) and the occupancy grid come from the same solid, so the three fields agree. The wire format of all three is unchanged. Implementation: the in-box points are min-scattered into a bottom view `(u, w)` (the lowest height per column is the underside there, the smallest reach its front edge), each column's solid spans the rows from `height + slab` down to `height`, a `rows × cols × wcells` occupancy is filled by comparing row indices against those bounds and its first occupied reach cell per `(row, col)` is the scan (`--front-wcells`, default 48, bins the reach for the extrusion and the voxels; the scan keeps exact reaches).

**Cost.** On the development desktop with a 480x360 frame and a hand of ~28 000 points: the front view takes ~3.9 ms at 160x120 with 48 reach cells (2.6 ms with 32, 4.0 with 64), the model's front and back faces ~1.8 ms per hand at that size, and the whole `analyze` pass ~13 ms without fusion, ~14.5 ms with it (image mode with fusion: ~16 ms).

**Hello.** `frame: "upright"` (image mode omits the field) and `box` in metres: `x` across the device (`[X0 − W/2, X0 + W/2]`), `y` the height range (`[near, far]`) and `z` the reach toward the display (`[Z0 − D/2, Z0 + D/2]`):

```json
{"type":"hello","version":1,"source":"leap","box":{"x":[-0.31,0.31],"y":[0.1,0.45],"z":[-0.175,0.175]},"frame":"upright","fps":30.0,"occupancy":{"width":32,"height":24},"voxels":{"nx":32,"ny":24,"nz":16},"surface":{"width":160,"height":120},"skeleton":true}
```

**Intrinsics.** `FrameSource.intrinsics` is `(fx, fy, cx, cy)` in pixels of the depth image, `cx`/`cy` in edge coordinates (pixel `i` spans `[i, i + 1)`). The Leap sources take them from the rectified view (`f = W / (2 tan(fov/2))`, centred), turned with `--leap-orient`; the synthetic source has a virtual 90° pinhole so upright mode runs and is tested offline; the RealSense reads them off its depth stream profile once started (`ppx + 0.5`, scaled for `--decimation`; not checked against hardware here). `--frame upright` with a source that has none stops with a message saying so.

| Flag | Meaning |
| --- | --- |
| `--frame image\|upright` | `image` (default): the box in image terms, as above. `upright`: the metric box of this section. |
| `--box-mm W D` | Upright box across the device and toward the display, mm (default `620 350`). |
| `--box-center X Z` | Where the upright box's centre sits, mm across and along the reach axis from the optical axis (default `0 0`). |
| `--reach-sign +1\|-1` | Which image direction faces the display: image-down (`+1`, default) or image-up (`-1`). |
| `--slab-mm MM` | How far the measured underside is extruded upward to make the solid (default 30). |
| `--front-wcells N` | Reach cells of the front-view solid, 1..256 (default 48). |

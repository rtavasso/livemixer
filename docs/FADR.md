...
Before building the set, every stem is decoded and rendered to FLAC in `rendered-stems/` beside the stems folder. All stems of a song get exactly the same length: the latest point where any of its stems is above `--silence-db` (default −60 dBFS). Longer stems are cut there, with a 10 ms fade, and shorter stems are padded with silence. Songs are then placed back to back with no gap (`--gap-bars N` restores a bar-aligned gap). Renders are cached per song in `_render.json` and reused while the source files and threshold are unchanged, so reordering is fast. Rendering all 19 Groop Show songs takes about four minutes and 1.6 GB.

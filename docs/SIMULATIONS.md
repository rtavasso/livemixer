...
| `quality=low\|medium\|high` · `dpr=1` · `rate=30` | Simulation detail and pixel budget, pixel-ratio cap, telemetry rate. |
...
- **Budget for a 2020 Intel MacBook** (integrated GPU, Safari or Chrome). Prefer RGBA16F targets (`pickFormat`), 256²–512² fluid grids, a few thousand cloth points at most, few full-screen passes within the host pixel budget. `low` caps the canvas at 480,000 pixels, `medium` at 1,024,000, and `high` at 2,073,600, independently of Retina scaling. Keep CPU work small enough to leave headroom for audio; the overlay measures CPU submission, not GPU execution. Use `npm run benchmark:sim` to measure delivered frame cadence on the real renderer.
...
### Veil (`veil`)
A sheer curtain hanging inside the volume at a plane `plane · depth` from the glass, breathing in daylight from a divided window on the back wall. CPU position-based dynamics (40×60 / 52×78 / 56×84 points by quality, 2 substeps) with structural, shear and bend constraints, gathered folds, a curl-noise wind lattice with gusts. Contact is geometric against whatever solid is known: with a depth-camera scan, the scanned front becomes a shell 0.08 deep that the fabric cannot enter (points inside leave by the nearer face, tilted by the local slope), so a real hand pushes the sheet ahead of it and the sheet can rest on its back; with a skeleton, every capsule collides on its own, so single fingers poke narrow pockets, a flat palm a wide one, a fist a small one, with friction and drag per capsule; with only a position, a sphere. In front of the plane a solid does not touch the sheet, at the plane it presses into it, deeper than the plane it pulls the fabric through and shows as a soft silhouette against the window (fingers appear before the palm), and it casts a shadow onto the fabric from the off-axis window. The rod is sized so the sheet fills the view at its depth; the room (floor at y = 0, back wall with the window) is ray-cast in the background pass. Rendered with the shared window camera as an indexed mesh with premultiplied blending: coverage rises where folds are seen edge-on, warm transmission from the backlight, separately coloured fibres, a Charlie sheen lobe, anti-aliased weave, layered selvedges and stitched hems. The room has a divided daylight window and a grain-textured floor; a shared filmic curve finishes the linear-light image. Params: `wind`, `gustiness`, `stiffness`, `damping`, `drape`, `opacity`, `backlight`, `tint`, `fabric`, `sheen`, `plane`, `weave`. Signals: `sway`, `flutter`, `contact` (share of fabric within a hand's reach), `depth` (mean sheet displacement along z: −1 toward the viewer … +1 deeper), `gust`, `tension`. The physical grid is unchanged; the extra fibre detail is shading, so it does not add cloth constraints. See [natural materials and performance](NATURAL-MATERIALS.md).

### Basin (`basin`)
Clear water and absorbing pigment in a glazed porcelain bowl on a stone table, viewed through a perspective camera at 40° above the table by default. The physical water plane remains x/depth, and height decides what is in the water; changing `elevation` (35–90°) only changes the view. Stam stable fluids on a 160²/256²/320² grid (dye at twice that) in an analytic circular bowl with no-slip walls: advection, forces from whatever is submerged, vorticity confinement, Jacobi pressure (16/24/32 iterations), dye advection with dissipation (bounded MacCormack correction at medium/high to preserve fine trails), a faint drift so ink keeps moving when nobody is there. The solid decides the stirring: with a depth-camera scan, the scanned cells below the water level form the wet footprint and drag the water with the scan's motion; with a skeleton, each capsule crossing the surface contributes its own stadium-shaped footprint (five dipped fingertips make five small stirs, a flat palm one broad push, a fist one round one, with finger motion adding to the palm's), and immersion is the submerged share of the hand's length; with only a position, a disc. A hand above the surface casts a height-coded shadow shaped like the solid (large and soft when high, small and crisp when low) and a lit waterline ring around every part that crosses the surface; a fast downward crossing plunges a bead of ink with a radial impulse, a slow dip drops a gentle bead (`dropOnEnter`). An independent damped wave field runs in one 96²/128²/160² pass per fixed step. Drops and wet hand motion displace the surface, waves reflect from the wall and dissipate. Signed 16-bit height and velocity are packed into RGBA8 on every device. The composite derives normals and bounded caustic focusing from that field, refracts both pigment and the ceramic floor, and applies Beer–Lambert colour absorption, a sharp Fresnel window reflection, curved inner floor and outer bowl, rounded ceramic lip and stone texture. Analytic intersections establish depth and occlusion; subpixel samples smooth the ceramic silhouette and lip. The deeper submerged interior carries irregular wheel marks, fine iron speckles, mottled glaze and small normal variations, all evaluated at the refracted floor position. The wall shadows light paths outside the opening, while water attenuation follows optical depth; texture detail fades below a pixel. Ink density is never used as surface height. Params: `viscosity`, `vorticity`, `stir`, `ink`, `palette` (indigo, ember, lagoon, sumi, orchid), `fade`, `bowl`, `surface` (water level as sim y), `dropOnEnter`, `light`, `caustics`, `ripples`, `glaze`, `ceramicTexture` (zero for smooth glaze), `elevation`. Signals from a 16×16 probe read back asynchronously: `energy`, `swirl`, `rotation` (−1 clockwise … +1 counter-clockwise), `ink`, `calm`, plus `immersion` (how much of the primary hand is under the surface). Cost: 31 fluid/dye passes plus one small wave pass per fixed step, one composite and the asynchronous probe per rendered frame at `medium`.

### Prism (`prism`)
White light and glass. A vertical glass prism stands in the middle of the volume; light travels in a horizontal plane at the hand's height, from the hand's position in that plane toward the prism, so walking around the prism (including pushing in behind it) changes the incidence angle and the spectrum fans out, folds and reflects internally; raising or lowering the hand raises the sheet of light, above the glass it passes over; an open hand gives a wide beam, a fist a narrow one; with nobody there a faint idle beam orbits at mid height. Bodies in the volume occlude the light: the scan's cells at the plane's height (or, without a scan, the exact slices of the skeleton's capsules) stop rays, which splash warmly on the skin, so a second hand held into the fan throws a shadow through the spectrum and fingers slice it; the beam's own hand is the emitter and clears its palm before the first segment. The solids themselves are drawn as dim rim-lit ghosts at their real heights. CPU ray tracer (`optics.ts`, unchanged 2D maths in the x/z plane): Cauchy dispersion, Snell, unpolarised Fresnel splitting with a depth-first stack, total internal reflection, 12/16/24 wavelengths per ray from an interleaved spectrum table, capped at 40 000 segments with an adaptive ray budget that thins the beam uniformly. Rays end on the volume's walls and leave a soft splash of colour there; the beam's footprint glows faintly on the floor. Rendering: the segments are 3D world points projected in the vertex shader with a window camera whose eye is raised (`eyeY` 1.1) so the horizontal light plane is seen from slightly above rather than edge-on; anti-aliased additive line quads into RGBA16F, two glow blurs, closed glass intersected analytically against the same triangular planes as the beam tracer. Camera rays cross entry/exit faces with Fresnel reflection and thickness-dependent absorption, with at most two internal segments. Studio cards reflect in polished faces and a narrow bevel; the stone table refracts through the solid. This replaces additive glass faces and the floor grid without adding a framebuffer or pass. Params: `size`, `height`, `spin`, `glass`, `dispersion`, `beam`, `rays`, `bounces`, `glow`, `roughness`, `floor`, `idle`, `twin`. Signals: `spread`, `hue`, `brightness`, `reflected`, `incidence`, `inside`, `elevation` (height of the light plane), `occluded` (share of beam energy stopped by bodies). Cost: trace ≈1.3–2.3 ms at `medium` with two hands and a scan, once per rendered frame.

### Shallows (`shallows`)
Open water over sand, seen from straight above. There is no vessel: the water fills the screen and its only walls are the four screen edges, so whatever a hand sets going runs out to them, reflects, and the whole sheet sloshes until it settles. The water is the x/depth plane. By default a hand moves the water wherever it is in the volume; with `heightContact` on, the hand's height decides contact as in Basin (`level`): a hand above the surface only casts a shadow through the water onto the sand; crossing the surface going down splashes (harder when faster); a submerged hand that moves piles water up ahead of itself and leaves a wake, and pressing down or lifting out displaces the water around every wet part. The solid is Basin's (`basin/model.ts`: scan first, then each skeleton capsule as its own footprint, so dipped fingers leave narrow wakes and a flat palm a broad one, then a sphere), carried onto the full-screen plane as up to 32 "stamps" per step. Physics (`model.ts`, `shaders.ts` `wave`): one height/rate field of square cells (192/256/384 rows by quality, the columns follow the canvas aspect; RGBA32F or RGBA16F, or two signed 16-bit values packed in RGBA8) obeying `h_tt = c² ∇²h − settle·h_t + ν ∇²h_t` with an isotropic nine-point Laplacian, stepped with symplectic Euler in as many substeps as keep the Courant number under 0.7 (3–8 per fixed step). The ν term damps a wave at ν k²/2, so short ripples die within a couple of seconds (`ripples`) while only `settle` decides how long the long slosh rocks. Both forcing kernels integrate to zero: hands move water, they never create it. The packed path reads whole texels and rounds symmetrically about zero; either bias otherwise lifts the entire surface over a minute. Optics, one composite pass: the view ray is refracted at the surface to find the bed point (the sand swims under every crest); sunlight refracted the same way maps the surface onto the bed as q(p) = p − a∇h, and the light arriving is the inverse area ratio 1/|det(I − a·Hessian h)|, which draws true caustic lines where waves focus, with red, green and blue bent slightly differently (`dispersion`). On top of the simulated field rides a `breeze` swell: ten travelling sines (exact solutions of the same linear equation, so they superpose), whose analytic slope and curvature are added at full screen resolution; they are what draws the moving caustic network when nobody is there, and hand-made waves tear through it. The bed is not level (pools and bars change the focus and the tint across the screen) and is rippled sand: forking ripple marks shaded by the sun, dark minerals in the troughs, grain, flecks and a few half-sunk pebbles. Beer–Lambert absorption over both legs plus in-scattered light tints deeper water; skylight fills troughs and shadows cool against the warm lines; hand shadows and waterline rings are Basin's, evaluated at the refracted bed point so they wobble with the water. Params: `heightContact`, `level`, `force`, `speed`, `ripples`, `settle`, `breeze`, `depth`, `refraction`, `caustics`, `dispersion`, `light`, `clarity`, `water`, `sand`, `sandTexture`. Signals from a 16×16 probe read back asynchronously (`gl/readback.ts`): `energy` (mean |h_t|), `sloshX` / `sloshZ` (signed tilt of the whole sheet across the screen and in depth: they swing for seconds after a sweep, which makes them good slow modulation sources), `ripple` (mean curvature: fresh short waves), `calm`, plus `immersion`. The swell is optics only and never reaches the signals, so `calm` really means nobody has touched the water. Cost: 3–8 small wave passes per fixed step, one composite and the probe per rendered frame; a steady 60 fps on an Intel Iris Plus 655 at `medium`.
...
- Intel Iris Plus 655 measurements and the repeatable benchmark are documented in [natural materials and performance](NATURAL-MATERIALS.md). Browser timing is workload-specific; camera inference and sustained thermal load still need installation testing.
- Simulations use capsule skeletons and the depth surface where available; the coarse `SimInput.occupancy` grid remains available to future simulations.
...
- `npm run benchmark:sim -- --port 4190 --seconds 20` — headed Chromium, actual renderer, delivered frame percentiles, CPU submission, signal ranges, screenshots and JSON report in `shots/benchmark/`. Headless/software results are explicitly labelled.
- `tests/sim-materials.spec.ts` — GPU wave propagation, rest and damping, plus Retina pixel budgets and quality switches.

...
A scan needs a depth camera. The Leap Motion Controller is a stereo infrared pair with no depth output, and its service does not expose the raw images to the browser; see "Leap Motion Controller" below for what it can and cannot provide.

### Solid hands from a skeleton

When a source knows the skeleton instead (the Leap), a hand is still not only a point. Sources that know the skeleton (the Leap: five fingers with four joints and widths each, palm width, wrist, elbow, forearm width) supply `capsules`, and the tracker forwards them on every `HandState` as `hand.capsules`: finger bones, the five metacarpals that make the palm, and a forearm cut off 14 cm past the wrist, each a segment with a radius. They ride on the smoothed position, so they never jitter against it. Sources that only know a position leave `capsules` empty and simulations fall back to a sphere of `radius` at `position`.

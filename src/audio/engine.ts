...
import { createDeck, disposeDeck, rampRecipe, scheduleDeckExit, setFilterBypass, updateFilter, updateSpace, type Deck } from './deck';
...
export class AudioEngine {
  ...
  space = idleSpace();
  private lastCommandId = 0; private lastBeatKey = ''; private clicks: { oscillator: OscillatorNode; at: number }[] = [];
  ...
  execute(action: AudioAction) {
    ...
    if (action.type === 'StartTransport') {
      ...
      this.decks.push(createDeck(this.context, this.assets.scenes[action.sceneId], this.master, action.at, action.recipe, this.openness, 10, this.bypass, action.rate ?? 1, this.space));
      this.lastCommandId = action.id; return;
    }
    if (action.generation !== this.generation || action.id <= this.lastCommandId) return;
    if (action.type === 'SetPlaybackRate') {
      this.requireLead(action.at);
      const deck = this.decks.find(d => d.loaded.scene.id === action.sceneId && d.stopAt === undefined);
      if (!deck) throw new Error('Playback deck is no longer available.');
      for (const stem of Object.values(deck.stems)) stem.source.playbackRate.setValueAtTime(action.rate, action.at);
      deck.space.setRate(action.rate, action.at);
      for (const click of this.clicks) if (click.at >= action.at) { try { click.oscillator.stop(action.at); } catch { /* ended */ } }
      this.lastBeatKey = '';
    } else if (action.type === 'RampRecipe') {
      ...
      const incoming = createDeck(this.context, this.assets.scenes[action.to], this.master, action.at, action.recipe, this.openness, (action.fadeInEnd - action.at) * 1000, this.bypass, action.rate ?? 1, this.space);
  ...
  setSpace(state: SpaceState, now = this.context.currentTime) {
    if (JSON.stringify(state) === JSON.stringify(this.space)) return;
    this.space = { ...state }; for (const deck of this.decks) updateSpace(deck, state, now);
  }
  ...
  tickMetronome(state: TransportState, now: number) {
    ...
    if (key === this.lastBeatKey || at > now + .15 || (state.committedAdvance && at >= state.committedAdvance.at) || (state.committedRate && at >= state.committedRate.at)) return;
    ...
    osc.connect(gain).connect(this.master); osc.start(at); osc.stop(at + .03); this.clicks.push({ oscillator: osc, at });
    osc.onended = () => { osc.disconnect(); gain.disconnect(); this.clicks = this.clicks.filter(c => c.oscillator !== osc); };
  }
  stop(now = this.context.currentTime) {
    ...
    for (const click of this.clicks) { try { click.oscillator.stop(now); } catch { /* ended */ } click.oscillator.disconnect(); }
    this.clicks = [];
  }

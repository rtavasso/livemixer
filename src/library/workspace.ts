...
interface Callbacks { beforeAudition: () => void; loadPerformance: (manifest: Manifest, read: ReadMedia, edge?: string) => Promise<void>; error: (error: unknown) => void }
...
export class LibraryWorkspace {
  ...
  constructor(readonly context: AudioContext, readonly callbacks: Callbacks) {
    const app = element('app'), header = app.querySelector('header')!, instrument = document.createElement('div'); instrument.id = 'instrument-view';
    for (const child of Array.from(app.children)) if (child !== header) instrument.append(child);
    const nav = document.createElement('nav'); nav.className = 'surface-tabs'; nav.setAttribute('aria-label', 'Workspace');
    nav.innerHTML = '<button id="instrument-tab" class="active" aria-pressed="true">Instrument</button><button id="library-tab" aria-pressed="false">Library & mix builder</button>';
    app.append(nav, instrument); const root = document.createElement('div'); root.id = 'library-view'; root.hidden = true; app.append(root);
    root.innerHTML = `
    <section class="panel library-intro"><div><h2>Song library</h2><p class="subtle">Inspect your stems, select synchronized passages, and connect them into a reviewed mix.</p></div><div class="actions"><label class="file-button">Choose library folder<input id="library-folder" type="file" webkitdirectory multiple /></label><button id="library-prepared" hidden>Load prepared song</button><button id="library-fixtures">Try test library</button><button id="library-save">Save project</button><label class="file-button">Restore project<input id="library-project" type="file" accept=".json" /></label></div></section>
    <div id="library-error" role="alert" hidden></div>
    <div class="library-toolbar"><input id="library-search" type="search" placeholder="Find a song…" aria-label="Find a song" /><span id="library-count" class="subtle">No songs imported</span><button id="analyze-all">Analyze all songs</button><button id="analyze-cancel" disabled>Cancel analysis</button></div>
    <p id="analysis-progress" class="subtle" aria-live="polite">Choose your external folder. WAV files stay on your computer.</p>
    ...
    <section class="panel mix-builder" aria-label="Mix path"><div class="section-heading"><div><h2>Mix path <span id="clip-count" class="subtle">0 passages</span></h2><p class="subtle">Each connection fades the outgoing scene to zero before the next begins at its native tempo. Reorder passages to explore the flow.</p></div><label><input id="path-repeat" type="checkbox" /> Repeat path</label></div>
    <div id="mix-path"><p class="subtle">Add a reviewed-length passage from the inspector to begin.</p></div><div class="actions"><button id="mix-load" class="primary" disabled>Open mix in instrument</button><button id="mix-export" disabled>Export playable mix ZIP</button></div><p id="mix-status" class="subtle">Exports contain aligned WAV excerpts and an unapproved manifest. Complete musical review in the instrument.</p></section>`;
    button('instrument-tab').onclick = () => this.show('instrument'); button('library-tab').onclick = () => this.show('library');
    ...
    handle('mix-load', () => this.loadMix()); handle('mix-export', async () => {
      ...
      try { const clips = [...this.clips], manifest = clipsManifest(clips, input('path-repeat').checked, this.edgeNotes, this.performance); element('mix-status').textContent = 'Packing aligned WAV excerpts…'; download('livemixer-mix.zip', await exportMixZip(manifest, clips)); element('mix-status').textContent = 'ZIP exported. Extract it and open its folder in the instrument.'; }
  ...
  configurePrepared(base: URL) {
    ...
    trigger.onclick = async () => {
      ...
      try {
        ...
        this.status('Prepared song loaded with full-length waveforms, saved analysis, and a draft passage. Adjust the grid and audition before approving it.');
      } catch (error) { this.error(error); this.status('Prepared song could not be loaded. Previous library retained.'); }
      finally { this.busy = false; trigger.disabled = false; this.renderList(); this.renderPath(); }
    };
  }
  show(surface: 'instrument' | 'library') {
    element('instrument-view').hidden = surface !== 'instrument'; element('library-view').hidden = surface !== 'library';
    for (const name of ['instrument', 'library']) { button(`${name}-tab`).classList.toggle('active', surface === name); button(`${name}-tab`).setAttribute('aria-pressed', String(surface === name)); }
  ...
  private renderList() {
    ...
    button('analyze-all').disabled = !this.songs.length || this.analyzing || this.busy; button('analyze-cancel').disabled = !this.analyzing;
  }
  ...
  private renderDetail() {
    ...
    <div class="waveforms">${STEMS.filter(id => s.stems[id]).map(id => { const stats = a?.stems[id]; return `<div class="waveform-row"><div><label><input type="checkbox" data-solo-stem="${id}" ${this.active.has(id) ? 'checked' : ''} /> ${id}</label><button class="solo-button" data-solo="${id}">Solo</button><span class="subtle">${stats ? `Peak ${db(stats.peakDbfs)} · RMS ${db(stats.rmsDbfs)} dBFS · crest ${stats.crestDb.toFixed(1)} dB · ${(100 * stats.silentFraction).toFixed(0)}% quiet${stats.clippedSamples ? ` · ${stats.clippedSamples} near-full-scale samples` : ''}${stats.nonfinite ? ` · ${stats.nonfinite} NONFINITE` : ''}` : 'Analyze to see waveform and levels'}</span></div><canvas id="wave-${id}" width="1000" height="70" aria-label="${id} waveform; click to choose a start bar"></canvas></div>`; }).join('')}</div>
    <div class="selection-grid"><label>Grid BPM<input id="library-bpm" type="number" min="20" max="300" step=".1" value="${chosenBpm}" /></label><label>Downbeat offset, s<input id="library-offset" type="number" min="0" step=".001" value="${s.gridOffset}" /></label><label>Start bar<input id="library-bar" type="number" min="1" step="1" value="${s.startBar}" /></label><label>Loop length<select id="library-bars"><option value="4" ${s.loopBars === 4 ? 'selected' : ''}>4 bars</option><option value="8" ${s.loopBars === 8 ? 'selected' : ''}>8 bars</option></select></label><label>Key annotation<input id="library-key" type="text" value="${escapeHtml(s.key ?? '')}" placeholder="Optional; reviewed by ear" /></label></div>
    <p id="selection-status" class="subtle"></p><div class="actions"><button id="library-audition">Audition loop</button><button id="library-stop">Stop audition</button><button id="library-add" class="primary">Add passage to mix</button><label><input id="library-click" type="checkbox" ${this.metronome ? 'checked' : ''} /> Grid metronome</label><label><input id="library-vocals" type="checkbox" ${!s.stems.vocals ? 'disabled' : ''} /> Include vocals (whole-phrase changes)</label></div><p class="subtle">Set the first downbeat by ear. All exports use the same frame range in every stem, without changing pitch or tempo.</p>
    <label class="notes">Song / passage notes<textarea id="library-notes">${escapeHtml(s.notes)}</textarea></label>
    ...
    button('library-audition').onclick = () => this.audition().catch(e => this.error(e));
  ...
  stopAudition() {
    ...
    this.metronomeGain?.disconnect(); this.metronomeGain = undefined;
  }
  ...
  private async audition() {
    const s = this.song; if (!s) return; this.readSelection(); this.stopAudition(); this.callbacks.beforeAudition(); const generation = this.auditionGeneration; await this.context.resume();
    ...
    for (const id of STEMS) if (buffers[id]) {
      const source = this.context.createBufferSource(), gain = this.context.createGain(); source.buffer = buffers[id]!; source.loop = true; source.playbackRate.value = 1;
    ...
    const click = this.context.createBufferSource(); click.buffer = clickBuffer; click.loop = true; this.metronomeGain = this.context.createGain(); this.metronomeGain.gain.value = this.metronome ? .05 : 0;
    click.connect(this.metronomeGain).connect(this.context.destination); this.sources.push(click);
    this.sources.forEach(s => s.start(at, 0)); element('selection-status').textContent = 'Audition looping · all stems remain in phase while solo/mute changes their gains · −12 dB audition gain per stem';
  }
  private renderPath() {
    ...
    if (this.clips.length && !this.busy) element('mix-status').textContent = `${durationText(seconds)} minimum path residence · ${(bytes / 1024 / 1024).toFixed(1)} MiB WAV excerpts · reviews and media fingerprints are verified in the instrument. Individual scenes can loop longer during performance.`;
  }

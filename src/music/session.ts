...
import { desireRecipe, dropUncommitted, idleTransport, plan, requestAdvance, startTransport, stopTransport, type AudioAction, type PlannerEnvironment, type RequestSource, type TransportState } from './planner';
...
export class PerformanceSession {
  ...
  dispatch(input: SessionInput, atMs: number, audioTime: number): AudioAction[] {
    ...
    if (input.type === 'frame') {
      ...
      if (this.mode !== 'timbre_only' && this.manualRecipeValue === null) {
      ...
      const result = advanceGesture(this.gesture, { u: this.conditioned.smooth, atMs, valid, enabled: this.holdEnabled, eligible: this.state.running && audioTime >= this.state.clock.start + this.state.clock.duration, busy: !!(this.state.pendingAdvance || this.state.committedAdvance) }, settings);
    ...
    if (input.type === 'tick') {
      const result = plan(this.state, this.environment, audioTime); this.state = result.state;

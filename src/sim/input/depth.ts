  ...
  private connect(generation: number) {
    ...
    socket.onmessage = event => {
      ...
      try {
        ...
        if (message.type === 'hello') { this.hello = message; this.state = { state: 'running', message: `Bridge "${message.source}" ready${message.fps ? ` at ${message.fps} fps` : ''}${message.skeleton ? ', with hand skeletons' : ''}${message.frame === 'upright' ? ', upright frame (camera looking up)' : ''}.` }; return; }

 ...
 */
import { z } from 'zod';
import type { Box3, HandObservation, OccupancyGrid } from './types';
...
export function mapObservation(mapping: SpaceMapping, o: HandObservation): HandObservation {
  return { ...o, position: mapPoint(mapping, o.position), extent: o.extent ? mapBox(mapping, o.extent) : undefined, points: o.points?.map(p => mapPoint(mapping, p)) };
}

 ...
 *   3. composite     — full-res: exposure, bloom, ACES tone-map, gamma, grain,
 *                      then sparkles as additive point sprites on top.
...
import { HEAD_K, SparkleField, StrokeTracker, decayFor, floorFor, hueWander, inkTarget, reduceProbe, sparkColour, sparkleRate, type StrokeSegment } from './logic';
...
export default defineSimulation({
  id: 'trails',
  title: 'Afterglow',
  description: 'Black; a present hand leaves slowly fading trails of light, like drawing with a slow luminous brush in a dark room.',
  params: {
    ...
    brushSize: { kind: 'number', default: .05, min: .01, max: .2, step: .005, label: 'Brush size', description: 'Brush radius in uniform units (canvas height = 1) for a typical hand; scales with the hand blob, a little with speed, and swells with push.' },
  ...
  signals: {
    glow: { min: 0, max: 1, description: 'Mean displayed luminance of the picture, from a 16×16 probe of the accumulation.', smoothing: .1 },
  ...
  create(ctx) {
    ...
    gl.enableVertexAttribArray(aSize); gl.vertexAttribPointer(aSize, 1, gl.FLOAT, false, stride, 8);
    ...
    const segPos = new Float32Array(MAX_SEGMENTS * 4), segCol = new Float32Array(MAX_SEGMENTS * 4);
    let pendingDt = 0, time = 0, hue = 0, ink = 0, glow = 0, coverage = 0;

    return {
      step(input: SimInput, params) {
        time = input.time;
        hue = hueWander(params.hue, time, params.hueDrift, hueNoise);
        const segments = strokes.update(input.hands, aspect, input.dt, { brushSize: params.brushSize, brightness: params.brightness, saturation: params.saturation, hue });
        let energy = 0;
        for (const s of segments) {
          if (pending.length >= MAX_SEGMENTS) pending.shift();
          pending.push(s);
          energy += s.amplitude;
          if (params.sparkle > 0) sparkles.emit(s, params.sparkle * sparkleRate(s.length, s.radius, s.push, input.dt), sparkColour(s.r, s.g, s.b));
        }
        ink = approach(ink, inkTarget(energy, input.dt), input.dt, energy > 0 ? .1 : .3);
        sparkles.step(input.dt, time, params.drift, aspect);
        pendingDt += input.dt;
      },
      ...
      render(frame, params) {
        ...
        if (elapsed > 0 || pending.length) {
          // 1. Decay and deposit this frame's segments.
          const count = Math.min(MAX_SEGMENTS, pending.length);
          for (let i = 0; i < count; i++) {
            const s = pending[i];
            segPos[i * 4] = s.ax; segPos[i * 4 + 1] = s.ay; segPos[i * 4 + 2] = s.bx; segPos[i * 4 + 3] = s.by;
            segCol[i * 4] = s.r * s.amplitude; segCol[i * 4 + 1] = s.g * s.amplitude; segCol[i * 4 + 2] = s.b * s.amplitude; segCol[i * 4 + 3] = s.radius;
          }
          accum.write.bind();
          accumulate.use().texture('u_prev', accum.read.texture, 0)
            ...
            .f1('u_aspect', aspect).f1('u_headK', HEAD_K).i1('u_count', count).f4v('u_segPos', segPos).f4v('u_segCol', segCol);
          drawQuad(gl);
        ...
        if (sparkles.count > 0) {
          ...
          for (let i = 0; i < sparkles.count; i++) {
            const intensity = sparkles.intensity(i, time) * gain;
            vertexData[k++] = sparkles.x[i]; vertexData[k++] = sparkles.y[i]; vertexData[k++] = sparkles.size[i];
          ...
          points.use().f1('u_aspect', aspect).f1('u_pointScale', frame.height / 900);
      ...
      signals() {
        return { glow: clamp01(glow), ink: clamp01(ink), hue: clamp01(hue), coverage: clamp01(coverage), sparkles: sparkles.alive };
      },

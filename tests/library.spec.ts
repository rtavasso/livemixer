...
test('browse, analyze, audition, connect, export and load an external stem library', async ({ page }) => {
  ...
  await page.goto('/?fixtures=1&tools=1'); await expect(page.locator('#start')).toBeEnabled(); await page.locator('#setup-tab').click(); await page.locator('#library-fixtures').click();
  ...
  await page.locator('#library-project-tools > summary').click();
  ...
  await page.locator('#setup-tab').click(); await page.locator('#filter-max').fill('6200'); await page.locator('#apply-editor').click(); await expect(page.locator('#load-status')).toContainText('Ready');
...
test('library search, invalid region and smaller layout give useful feedback', async ({ page }) => {
  await page.setViewportSize({ width: 768, height: 1000 }); await page.goto('/?fixtures=1&tools=1'); await expect(page.locator('#start')).toBeEnabled(); await page.locator('#setup-tab').click(); await page.locator('#library-fixtures').click();

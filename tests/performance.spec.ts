...
test('normal launch uses Sun and keeps tone, vocals, passages and studio tools distinct', async ({ page }) => {
  ...
  await page.locator('#vocal-toggle').click(); await expect(page.locator('#current-recipe')).toHaveText('sparse', { timeout: 2000 });
  ...
  await page.locator('#setup-tab').click(); await page.locator('#setup-authoring > summary').click();
  ...
  await page.locator('#setup-tests > summary').click(); await page.locator('#load-fixtures').click();
